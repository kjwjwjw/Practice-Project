...
int deleteCount = publicnewsDAO.deletepublicNews(num, pass);

// 작업 작업 결과 판별 및 처리
// deleteCount > 0 일 경우 "public_News.jsp" 페이지로 이동(페이지번호 전달)하고
// 아니면, 자바스크립트를 사용하여 "글 삭제 실패!" 출력 후 이전 페이지로 돌아가기
if(deleteCount > 0) {
	response.sendRedirect("public_News.jsp?page=" + pageNum);
} else {
	%>
	<script>
		alert("글 삭제 실패!");	
