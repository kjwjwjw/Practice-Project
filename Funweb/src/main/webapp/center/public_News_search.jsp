...
int listCount2 = publicnewsDAO.selectSearchpublicListCount(search,searchType);

// => 총 게시물 수 // 페이지 당 표시할 게시물 수 + 0.9
int maxPage = (int)Math.ceil((double)listCount2 / listLimit );
...
ArrayList<publicNewsBean> publicnewslist = publicnewsDAO.selectSearchpublicNewsList(pageNum,listLimit, search,searchType);
...
<head>
<meta charset="UTF-8">
<title>center/public_News_search.jsp</title>
	...
	<h1>페이지번호 : <%=pageNum %></h1>
		...
		<nav id="sub_menu">
			<ul>
				...
				<li><a href="public_News.jsp">Public News</a></li>
...
<!-- 				페이징 처리 시 링크에 notice.jsp 대신 notice_search.jsp  -->
				...
				=> 클릭 시 현재 페이지 번호(pageNum) -1 값을 파라미터로 전달
				 -->
				 <%if(pageNum>1) { %>
				 	<a href="public_News_search.jsp?page=<%=pageNum -1%>&search=<%=search %>&searchType=<%=searchType%> ">Prev</a>
				 <%} else { %>
					Prev&nbsp;
				 <%} %>
				 ...
				 <% for(int i=startPage;i<endPage;i++) {%> 
					...
					<%if(pageNum == i) { %>
						&nbsp;&nbsp;<%=i %> &nbsp;&nbsp;
					<%} else { %>
						<a href="public_News_serach.jsp?page=<%=i%>&search=<%=search %>&searchType=<%=searchType%>"><%=i %></a>
					<%} %>
				...
				<%if(pageNum<maxPage) { %>
				 	<a href="public_News_search.jsp?page=<%=pageNum +1%>&search=<%=search %>&searchType=<%=searchType%> ">Next</a>
				 <%} else { %>
					&nbsp;Next
				 <%} %>
			</div>
